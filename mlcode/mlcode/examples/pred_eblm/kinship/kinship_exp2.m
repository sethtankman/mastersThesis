%%%%%
% give x and the relationship, predict y that maximises relationship(x,y)
%%%%
clear all;
if ispc
...
EXP_DIR = strcat(HOME,['/WORK/experiments/DS/eblm/Kinship_exp2/' ...
                    'encode_ae/']);
...
for i=1:size(vld_dat,2)
    obj1 = find(vld_dat(1:obj_num,i));
    pred = find(vld_lab(:,i)); pred = pred(mod(pred,2)~=0); pred = ceil(pred(1)/2);
    obj2s = [];
    for j = 1:size(data,2)
        obj1_ = find(data(1:obj_num,j));
        pred_ = find(data(2*obj_num+1:end,j)); pred_ = pred_(mod(pred_,2)~=0); pred_ = ceil(pred_(1)/2);
        if obj1 == obj1_ && pred == pred_
            obj2s = [obj2s,find(data(obj_num+1:2*obj_num,j))];
        end
    end
    obj_lab = [obj_lab,obj2s];
end
    

for hidNum = [50,100,200]
conf.clause_h_num = hidNum;
for lr = [0.01,0.1,0.5,0.7,0.9]
conf.params(1)=lr;
conf.params(2)=lr;
for cv = [0.01,0.1,0.5,1,3,5,7,10]
    conf.initial_cv = cv;
    ...
    rs = [];
    for i = 1:size(vld_dat,2)
        in_  = [repmat(vld_dat(1:obj_num,i),[1,obj_num]);eye(obj_num)];        
        lab_ = repmat(vld_lab(:,i),[1,obj_num]);
        [~,out] = pred_eblm_infer(conf,model,in_,lab_);
        vld_lab_ = find(vld_lab(:,i));
        rel = vld_lab_(mod(vld_lab_,2)~=0); rel = rel(1);
        o2 = find(vld_dat(obj_num+1:2*obj_num,i));

        %%%% DEBUG
        %rel
        %for k=1:numel(obj_list)
        %    pred_list(ceil(rel/2))
        %    obj_list(find((vld_dat(1:obj_num,i))))
        % 
        %    obj_list(o2)
        %    obj_list(k)
        %    for t =1:size(out,1)
        %        fprintf('%.5f  %s\n',out(t,k),pred_list{ceil(t/2)})
        %    end
        %    pause
        %end
        %prd  = pred_list(ceil(rel/2))
        %o1 = obj_list(find((vld_dat(1:obj_num,i))))
        ...
        %obj_list(rank)
        %pause
        % check if the designate answer is among the correct ones?
